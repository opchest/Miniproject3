...
- Corner:
    Corner are the place that can't be flipped , so the corner is important is this game. So I count the number of corner captured by each side and then use `cal_ratio`.
```c++
